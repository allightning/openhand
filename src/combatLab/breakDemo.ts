...
function finishCut(): DemoLessonStep[] {
  return [
    play(["advance"], "上前", "打「进步」（1 劲）走进刀距。刀打 2 格。够不着打空。"),
...
export function demoLessons(stage: DemoStage, companion: CompanionId | null, track: "rookie" | "break" = "break"): DemoLessonStep[] {
  if (track === "rookie") {
    if (stage === 1) {
      return [
        play(["cut"], "出刀", "人已在刀距。打「斩」。左上角是劲。打完点「收势」：回满劲，再摸牌。够不着打空。"),
        end("收势", "点「收势」结束你的回合。他出手后你再打。", {
          title: "回合",
          body: "你出牌 → 点「收势」→ 他出手 → 你回满劲。先把这一套打顺。",
        }),
      ];
    }
    if (stage === 2) {
      return [
        play(["defend"], "格挡", "打「卸力」堆挡。他打在挡上先扣挡再扣血。"),
          ...
          title: "营地预告",
          body: "正式行路每场开战满状态。营地领免费奖励，黑市用彩金买。不下注大概够买 1 件最便宜的。",
        }),
      ];
    }
    if (stage === 3) {
      return [
        play(["retreat"], "走动", "打「撤步」改站位。距离会改他打不打得到你。"),
          ...
          title: "配装与下注",
          body: "牌包有张数上下限，不合规不能开打。下注开打前扣彩金。输了底彩不发，再抽 10% 出血。复活赛不能下注。",
        }),
      ];
    }
    return [
      play(["cut"], "再打一刀", "用「斩」收尾。低阶到此，破招去高阶新手关和训练馆。"),
      end("毕业", "点「收势」。之后可开程，或进高阶学破招。", {
        title: "低阶毕业",
  ...
  if (stage === 1) {
    return [
      play(["retreat"], "离开红格", "你踩在红格上。打「撤步」（0 劲）走出去，攒 1 点破招。"),
      end("收势硬拆", "点「收势」：人在红格外、有破招 = 硬拆。这一招作废，你得拆势。", {
        title: "硬拆给拆势",
        body: "硬拆 = 这一招作废 + 你得拆势（下一刀打真伤）。走出红格不点「收势」，什么也没有。",
      }),
      ...finishCut(),
    ];
  }
  if (stage === 2) {
    return [
      play(["retreat"], "第一段", "他这一回合连打两段。打「撤步」攒 1 点破招。拆 1 段扣 1 点，另一段照打你。"),
      end("看充能", "点「收势」。第 1 段硬拆作废。第 2 段没破招，照打你。破招按段扣。", {
        title: "破招按段扣",
        body: "硬拆 1 段扣 1 点破招。两段招要两张走位牌才拆得完。没拆的那段照打。",
      }),
      ...finishCut(),
    ];
  }
  if (stage === 3) {
    return [
      play(["defend"], "堆挡", "这回不走了。打「卸力」（1 劲）格挡 +8。点「收势」时格挡顶住他的伤害 = 让。"),
      end("让", "点「收势」。格挡顶住 = 让：伤减半，不得拆势。能走开硬拆就别站着让。", {
        title: "让不是反打",
        body: "让 = 格挡顶住落点，伤害减半，不得拆势。硬拆才是反打：走出红格 + 破招 + 点「收势」。",
      }),
  ...
  if (stage === 4) {
    return [
      play(["rift"], "破架", "他亮了架势。架势走不开也挡不住。打「开缝」（1 劲）攒破架。"),
      end("拆架", "点「收势」。破架拆他的格挡，你得拆势。角标「将破」才是硬拆。", {
        title: "招不同，拆法不同",
        body: "打击靠走出红格拆。架势靠破架牌拆。看角标：「将破」才是硬拆。",
      }),
  ...
  if (stage === 5) {
    return [
      play(["retreat"], "拆眼", "他第 1 段标了「眼」。打「撤步」走开，攒破招硬拆这一段。"),
      end("破眼", "点「收势」。硬拆眼段：后面几招全散，他失衡（承伤 ×2）。", {
        title: "眼 = 连招要害",
        body: "硬拆打中眼才破眼：后招全散、他失衡。让不算破眼。",
      }),
      ...finishCut(),
    ];
  }
  ...
  const head: DemoLessonStep[] = companion
    ? [
        swap(
          "换人",
          `点高亮「换人·${MATES[companion].name}」（1 劲）：${compWeapon}手换刀客上场。不想换就点「收势」跳过。`,
        ),
      ]
    : [];
  ...
  return [
    ...head,
    play(["retreat"], companion ? `${compWeapon}上场` : "拆招", "规则不变。打「撤步」离开红格，攒 1 点破招。"),
    end("再拆一次", "点「收势」。人在红格外、有破招 = 硬拆。规则没变，换的是兵器。"),
    ...closeOrThrust,
    end("收势结束", "点「收势」。这一局到此。", {
      title: "训练营到此",
      body: "硬拆、充能、让、破架、破眼、换人都试过了。追（拆「撤」）在训练馆。开程再自己配。",
    }),
  ];
}
