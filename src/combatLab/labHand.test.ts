...
describe("手牌上限 / 弃牌 / 置换", () => {
  it("超上限仍可出牌，但不能收势；点弃才压到上限", () => {
    ...
    const playable = b.hand.find((c) => canPlay(b, c.uid, contextNow()).ok && CARDS[c.defId]?.type === "attack");
    expect(playable).toBeTruthy();
    const afterPlay = playCard(b, playable!.uid, contextNow());
    expect(afterPlay.hand.length).toBeLessThan(b.hand.length);

    b = saberBattle();
    b.hand = [...b.hand, extra, { uid: "x2", defId: "direct" }];
    while (needsDiscardToHandCap(b, contextNow()) && b.hand[0]) {
  ...
  it("置换：弃 1 摸 1，每回一次，不占收势闸", () => {
    ...
    expect(labCanCycle(b, contextNow()).ok).toBe(false);
    expect(canEndPlayerTurn(b, contextNow()).ok).toBe(true);
  });
});
