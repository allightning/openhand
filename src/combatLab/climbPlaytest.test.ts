...
describe("爬塔试玩默认 2026-09-17", () => {
  ...
  it("手牌超上限不能收势；弃牌常亮可弃到上限", () => {
    ...
    expect(needsDiscardToHandCap(b, climbTestContext())).toBe(true);
    expect(canEndPlayerTurn(b, climbTestContext()).ok).toBe(false);
    b = labEnterDiscardPhase(b, climbTestContext());
    expect(b.climbDiscardPhase).toBe(true);
    while (needsDiscardToHandCap(b, climbTestContext()) && b.hand[0]) b = labDiscardCard(b, b.hand[0].uid, climbTestContext());
    expect(needsDiscardToHandCap(b, climbTestContext())).toBe(false);
    expect(canEndPlayerTurn(b, climbTestContext()).ok).toBe(true);
  });
  ...
  it("置换花费 = 牌费 − 1，可连换", () => {
    ...
    expect(labCanCycle(b, climbTestContext()).ok).toBe(true);
    const before = b.energy;
    b = labCycleCard(b, uid, climbTestContext());
  ...
  it("格挡收势不清，帽 12", () => {
    ...
    b.playerBlock = 20;
    const capped = endTurn(b, climbTestContext(), { deferIntentRefresh: true });
  ...
  it("弃牌阶段不能出牌", () => {
    ...
    b.energy = 20;
    const uid = b.hand[0]!.uid;
    labEnterDiscardPhase(b, climbTestContext());
    expect(canPlay(b, uid, climbTestContext()).ok).toBe(false);
    expect(canPlay(b, uid, climbTestContext()).reason).toContain("弃牌");
  });
  ...
  it("断劲：同手两枪 3–4 格扣敌劲", () => {
    ...
    expect(canPlay(b, "spk1", climbTestContext()).ok).toBe(true);
    ...
    expect(canPlay(b, "spk2", climbTestContext()).ok).toBe(true);
  ...
  it("连击重放花 2 层", () => {
    ...
    b.player.pos = 3;
    b.enemy.pos = 4;
    expect(labCanComboReplay(b, climbTestContext()).ok).toBe(true);
    b = labComboReplay(b, climbTestContext());
    // 花 2 → 1，重放命中再叠 +1 → 2
