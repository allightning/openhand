...
describe("§31.12 助战符（与同行分家的客座好手）", () => {
  it("召唤上场：实体占格、一回合后离场", () => {
    let b = v2Battle();
    const cells = legalSummonCells(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(cells.length).toBeGreaterThan(0);
    b = summonAssist(b, "staff", cells[0]!, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(b.labSummon?.name).toBe("顿僧");
    expect(b.labSummon?.pos).toBe(cells[0]);
    expect(occupied(b, cells[0]!, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }))).toBe(true);
    // 敌回合走完、玩家回合开始 → 离场
  ...
  it("六系绝活：棍眩晕 / 钩缴械 / 剑裂创 / 刀破绽 / 枪挑退", () => {
    let b = v2Battle();
    b = summonAssist(b, "staff", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(b.foeStun).toBe(1);

    b = v2Battle();
    b = summonAssist(b, "hook", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(b.foeDisarm).toBe(2);

    b = v2Battle();
    b = summonAssist(b, "sword", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(b.bleed).toBe(3);

    b = v2Battle();
    const ex = b.expose;
    b = summonAssist(b, "saber", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
    expect(b.expose).toBe(ex + 2);

    b = v2Battle();
    b.player.pos = 1;
    b.enemy.pos = 3;
    b = summonAssist(b, "spear", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
  ...
  it("拳符吸仇：敌攻击段打它，算你拆（势 +1）", () => {
    ...
    b.player.pos = 3;
    b.enemy.pos = 4;
    b = summonAssist(b, "palm", 2, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
  ...
  it("身位卡冲锋：召唤体挡住冲锋路 = 拆", () => {
    ...
    b.player.pos = 1;
    b.enemy.pos = 5;
    b = summonAssist(b, "sword", 3, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } })); // 冲锋路径中间
    const hpBefore = b.player.hp;
    const breaksBefore = b.v2BreakCount ?? 0;
  ...
  it("临时墙：敌被推上召唤体 = 撞墙（拳系震壁连招）", () => {
    ...
    b.player.pos = 3;
    b.enemy.pos = 4;
    b = summonAssist(b, "staff", 5, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } })); // 敌身后当墙
...
describe("§31.12 棋盘渲染：召唤体与点位高亮", () => {
  it("召唤体占格 + 点位模式高亮空格", async () => {
    ...
    let b = v2Battle();
    b = summonAssist(b, "palm", 0, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false } }));
