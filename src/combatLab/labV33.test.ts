...
function v2Battle(school: WeaponId = "palm"): Battle {
  const run = { ...createGauntletRun("shaolin", school), weaponId: `${school}-a-3` };
...
describe("§31.13 拆招 v4 · 以拆为杀", () => {
  ...
  it("硬拆一段（反架拆架式）：那段作废 + 拆势 + 势 +1", () => {
    ...
    expect(b.v2BreakMomentumTrue ?? 0).toBe(BREAK_COUNTER_BASE + 3);
    expect(b.qi ?? 0).toBe(1);
  });

  it("连环拆：一回合第 2 段硬拆叠两层拆势", () => {
    let b = v2Battle("sword");
    b.player.pos = 3;
    b.enemy.pos = 5;
    b.intents = [
  ...
  it("拆势打出能拆死人：敌血量低于真伤池时下一刀胜", () => {
    ...
    b.player.pos = 3;
    b.enemy.pos = 5;
    b = playCard(b, "a1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(b.phase).toBe("won");
  });

  it("破眼：套路崩塌 + 失衡 ×2 + 拆眼加力并入拆势", () => {
...
describe("§31.14 空间诚实 · 红格=结算，够不着的招不算拆", () => {
  ...
  it("打击收势出圈 = 劈空（不扣血、不得势、不算拆）", () => {
    ...
    b = playCard(b, "m1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 撤到 3，出圈
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
  ...
  it("本来就够不着的招：不算拆也不算让，自己打空", () => {
    ...
    b.energy = 3;
    const hp = b.player.hp;
    b = playCard(b, "m1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 哪怕出了位移牌（走到 2），也不算拆——他本来就打不到你
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(b.player.hp).toBe(hp);
  ...
  it("抢步扑锁定线：显示格=结算格；挪出身前 = 拆，本就圈外 = 抢空", () => {
    let b = v2Battle();
    b.player.pos = 3;
    b.enemy.pos = 4; // 贴脸：锁定格 3，不挪步，落点 4，身前 reach1 → [3]
    b.v2Turn = { ...b.v2Turn!, turnStartPos: 3 };
    const lunge: Intent = { kind: "lunge", damage: 12 };
    expect(dangerCellsForIntent(b, lunge, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }))).toEqual([3]);
    b.intents = [lunge];
    b.hand = [{ uid: "m1", defId: "backpalm" }];
    b.energy = 3;
    const hp = b.player.hp;
    b = playCard(b, "m1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 撤到 2，出圈 = 硬拆
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(b.player.hp).toBe(hp);
    expect(b.v2BreakCount ?? 0).toBe(1);

    // 本来就够不着：敌 5 你 1，抢步落 4、身前 [3]——不拆不让，他自己抢空
    let b2 = v2Battle();
    b2.player.pos = 1;
    b2.enemy.pos = 5;
    ...
    b2 = endTurn(b2, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
  ...
  it("§31.15 连抢投影：后一段红格按前一段落位推进——不再「不在红格仍挨打」", () => {
    // 敌 4 你 1，[抢步,抢步]：第一段落 3 身前 [2]；第二段落 2 身前 [1]
    ...
    const projected = projectedQueueThreat(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(projected[0]).toEqual([2]);
    expect(projected[1]).toEqual([1]);
    expect(dangerCells(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }))).toContain(1); // 并集含脚下（第二段）
    // 原地不动：第一段抢空，第二段命中
    const hp = b.player.hp;
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    ...
    b2.energy = 3;
    const hp2 = b2.player.hp;
    b2 = playCard(b2, "m1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 撤到 0
    b2 = endTurn(b2, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(b2.player.hp).toBe(hp2);
  ...
  it("§31.15 拆招战利品：走位拆起架（后手段吃到）+ 硬吃拆回血 + 连环", () => {
    ...
    b.intents = [
      { kind: "lunge", damage: 12 },
      { kind: "bleedcut", damage: 6, bleed: 1 },
    ];
    b.hand = [{ uid: "m1", defId: "backpalm" }];
    b.energy = 3;
    b.player.hp = b.player.maxHp - 10; // 压血让回血可见（满血会被上限吃掉）
    const hp = b.player.hp;
    b = playCard(b, "m1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 撤到 1（退步掌自带架 5）
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
  ...
  it("§31.15 撤步：0 费退 2，给拆招充能——退到底正好拆连抢第二段", () => {
    ...
    const hp = b.player.hp;
    b = playCard(b, "r1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 1 → 0（身后只剩 1 格）
  ...
  it("§31.15 拆招战利品：桩拆回劲 +2（借势回劲）", () => {
    ...
    b.energy = 5;
    b = playCard(b, "p1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } })); // 劲 5-1=4，桩落身前
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    expect(b.v2BreakCount ?? 0).toBe(1);
  ...
  it("§31.15 角色技能平衡：锡息 1→3 / 缆手 +2→+3 / 桩皮反震 +1→+2", () => {
    ...
    b2 = playCard(b2, "h1", makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
    ...
    b3 = endTurn(b3, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0 } }));
  ...
  it("应激不再当场报复：挂起到下一手，带应签入场", () => {
    ...
    b = endTurn(b, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 3 } })); // 结算 → 规划下一手
  ...
  it("单回合总督：攻击总伤封顶玩家气血上限比例，保留最大一段", () => {
    ...
    applyTurnDamageGovernor(b, q, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0, enemyTurnCapRatio: 0.45 } }));
    ...
    applyTurnDamageGovernor(b, small, makeTestContext({ mode: "break", lab: true, tuning: { rulesV2: true, v2Fx: false, enemySegBonus: 0, v2VariantAi: false, enemyStressCap: 0, enemyTurnCapRatio: 0.45 } }));
...
describe("§31.13/§31.14 赌馆 · 彩金与下注", () => {
  ...
  it("盘口轮换：馆 1 不开连拆/破眼，每场开三", () => {
    ...
    for (let i = 0; i < 48; i++) {
      ...
      expect(offers.some((o) => o.kind === "chain" || o.kind === "eye")).toBe(false);
      offers.forEach((o) => seen.add(o.kind));
    }
    expect(seen.size).toBeGreaterThanOrEqual(5);
    expect(seen.has("chain")).toBe(false);
    expect(seen.has("eye")).toBe(false);
  });
  ...
  it("连拆注：内核仍结算，盘口默认不开", () => {
    setLabRuleset("break");
    const wRun = { ...runWithPot(50), stage: 3, wager: { kind: "chain" as const, stake: 30, target: 3, odds: 2.5 } };
    const win = resolveWager(wRun, stats({ breaks: 3 }));
    expect(win.won).toBe(true);
    expect(win.payout).toBe(Math.round(30 * 2.5));
    const lose = resolveWager(wRun, stats({ breaks: 2 }));
    expect(lose.won).toBe(false);
    expect(lose.payout).toBe(0);
    for (let i = 0; i < 40; i++) {
      expect(wagerOffers({ ...runWithPot(50), stage: 5, items: ["jinchuang"] }, () => (i * 0.02) % 1).some((o) => o.kind === "chain" || o.kind === "eye")).toBe(false);
    }
  });

  it("破眼注：破眼次数达标才中（仅内核，盘口不开）", () => {
    setLabRuleset("break");
    const wRun = { ...runWithPot(50), wager: { kind: "eye" as const, stake: 10, target: 2, odds: 3 } };
    expect(resolveWager(wRun, stats({ eyes: 2 })).won).toBe(true);
    expect(resolveWager(wRun, stats({ eyes: 1 })).won).toBe(false);
  });
  ...
  it("完璧注/速胜注：输馆必飞；彩金不欠债（庄垫只亏到 0）", () => {
    const clean = { ...runWithPot(5), wager: { kind: "clean" as const, stake: 10, target: 75, odds: 1.5 } };
    ...
    expect(win.won).toBe(true);
    expect(win.payout).toBe(Math.round(10 * 1.5));
    const speedRun = { ...runWithPot(5), wager: { kind: "speed" as const, stake: 10, target: 6, odds: 2 } };
  ...
  it("盘口默认不开连破/破眼", () => {
    setLabRuleset("break");
    const late = runWithPot(50, 5);
    for (let i = 0; i < 40; i++) {
      const kinds = wagerOffers(late, () => (i * 0.03) % 1).map((o) => o.kind);
  ...
  it("§31.16 黑市摆摊：满血不卖金创；馆 1 不卖淬刃（闸未开）", () => {
    const fresh = runWithPot(60);
    const stall = marketOffers(fresh);
    expect(stall.some((o) => o.kind === "heal")).toBe(false);
    expect(stall.some((o) => o.kind === "forge")).toBe(false);
    expect(stall.length).toBe(3); // 谱/货/外功
    stall.forEach((o) => expect(o.price).toBeGreaterThan(0));
    const hurt = { ...fresh, hp: fresh.hpMax - 10 };
    const stall2 = marketOffers(hurt);
    expect(stall2.length).toBe(4);
    expect(stall2[0]!.kind).toBe("heal");
    const gated = { ...fresh, stage: 4, streak: 3, weaponId: "palm-a-1" };
    expect(marketOffers(gated).some((o) => o.kind === "forge")).toBe(true);
    const maxed = { ...gated, weaponId: "palm-a-4" };
    expect(marketOffers(maxed).some((o) => o.kind === "forge")).toBe(false);
  });

  it("§31.16 黑市落锤：扣彩金、货到手上；不够钱不卖", () => {
    const run = { ...runWithPot(200), weaponId: "palm-a-1", stage: 4, streak: 3 };
    // 金创：先砍到残血再买回
    const hurt = { ...run, hp: 30 };
    const heal = marketOffers(hurt).find((o) => o.kind === "heal")!;
    const healed = buyMarketOffer(hurt, heal)!;
    expect(healed.pot).toBe(200 - heal.price);
    expect(healed.hp).toBe(Math.min(hurt.hpMax, 30 + Math.ceil(hurt.hpMax * GAUNTLET_HEAL_RATIO)));
    // 谱：进牌组的就是摊上那张
    const stall = marketOffers(run);
    const card = stall.find((o) => o.kind === "card")!;
    const bought = buyMarketOffer(run, card)!;
    expect(bought.pot).toBe(200 - card.price);
