...
describe("爬塔结束/开始逐拍", () => {
  ...
  it("defer 收势后裂创/回劲/摸牌不立刻落地，逐拍才结算", () => {
    ...
    b = endTurn(b, { deferIntentRefresh: true, deferStatusTicks: true });
    ...
    const bleed = applyClimbPhaseBeat(b);
    ...
    if (peekClimbPhaseQueue(b)[0] === "drawBench") {
      const bench = applyClimbPhaseBeat(b);
      if (!bench.skip) {
    ...
    const intents = applyClimbPhaseBeat(b);
    ...
    expect(applyClimbPhaseBeat(b).done).toBe(true);
  });
  ...
  it("亮招文案用共 N 段，不用后手隐", () => {
    ...
    while (peekClimbPhaseQueue(b).length) {
      b = applyClimbPhaseBeat(b).battle;
    }
  ...
  it("亮招后手只挂旗不静默兑招", () => {
    ...
    const after = applyClimbPhaseBeat(b);
    b = after.battle;
