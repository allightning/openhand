...
describe("rewards", () => {
  it("applies an upgrade by replacing one copy", () => {
    ...
    const next = applyReward(run, { kind: "upgrade", from: "strike", to: "strike2" }, climbTestContext());
  ...
  it("applies a technique without touching the deck", () => {
    const run = makeRun("empty");
    const next = applyReward(run, { kind: "technique", id: "longPush" }, climbTestContext());
  ...
  it("rolls three unique picks after a duel", () => {
    const run = makeRun("empty");
    const picks = rollRewards(run, emptySave(), { type: "duel", enemyId: "catcher" }, climbTestContext());
  ...
  it("chest offers techniques first", () => {
    const run = makeRun("empty");
    const picks = rollRewards(run, emptySave(), { type: "chest" }, climbTestContext());
    expect(picks.length).toBe(3);
  ...
  it("does not offer a remnant already owned", () => {
    ...
    run = applyReward(run, { kind: "technique", id: ENEMIES.catcher.remnant }, climbTestContext());
    const picks = rollRewards(run, emptySave(), { type: "duel", enemyId: "catcher" }, climbTestContext());
  ...
  it("stashes a saber page instead of swapping a palm card", () => {
    const run = makeRun("empty");
    const next = applyReward(run, { kind: "replace", from: "strike", to: "cut" }, climbTestContext());
  ...
  it("always offers a challenge remnant after a kill", () => {
    const run = makeRun("empty");
    const picks = rollRewards(run, emptySave(), { type: "duel", enemyId: "bandit" }, climbTestContext());
  ...
  it("teaches a stashed saber page when a saber hand is already in the party", () => {
    ...
    const next = applyReward(run, { kind: "add", id: "cut" }, climbTestContext());
