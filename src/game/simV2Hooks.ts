...
export function simV2ApplyFinisher(
  b: Battle,
  _defId: string,
  baseDmg: number,
  rc: RunContext,
...
export function simV2ResolveIntentQueue(
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    const blk0 = b.playerBlock;
    const recapSeg = (outcome: string) => {
      recap.push({
        ord: i + 1,
        name,
        outcome,
        hpLost: Math.max(0, hp0 - b.player.hp),
        blockLost: Math.max(0, blk0 - b.playerBlock),
      });
    };
    if (collapsed) {
