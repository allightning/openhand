...
function playNamed(b: ReturnType<typeof makeTutorialBattle>, name: CardId) {
  ...
  return playCard(b, card.uid, makeTestContext({ mode: "climb", lab: false }));
}

describe("tutorial battle", () => {
  it("deals a fixed first hand", () => {
    const b = makeTutorialBattle(makeTestContext({ mode: "climb", lab: false }));
  ...
  it("three palm strikes leave the catcher alive", () => {
    let b = makeTutorialBattle(makeTestContext({ mode: "climb", lab: false }));
  ...
  it("push from the sixth step hits the wall for 8", () => {
    ...
    const prev = previewCard(b, b.hand.find((c) => c.defId === "push")!.uid, makeTestContext({ mode: "climb", lab: false }));
  ...
  it("preview equals settlement for strike", () => {
    ...
    const prev = previewCard(b, strike.uid, makeTestContext({ mode: "climb", lab: false }));
    const after = playCard(b, strike.uid, makeTestContext({ mode: "climb", lab: false }));
  ...
  it("turn two can charge then strike for 10", () => {
    ...
    b = playCard(b, strike.uid, makeTestContext({ mode: "climb", lab: false }));
...
describe("other duelists", () => {
  it("escort charges into the player and deals 18", () => {
    let b = makeBattle("escort", makeTestContext({ mode: "climb", lab: false }));
  ...
  it("piler drops a stake that blocks knockback", () => {
    let b = makeBattle("piler", makeTestContext({ mode: "climb", lab: false }));
    ...
    b = playCard(b, push.uid, makeTestContext({ mode: "climb", lab: false }));
...
describe("new verbs and techniques", () => {
  it("hauler pulls the player closer", () => {
    let b = makeBattle("hauler", makeTestContext({ mode: "climb", lab: false }));
  ...
  it("delay telegraphs windup then a heavy strike", () => {
    let b = makeBattle("delay", makeTestContext({ mode: "climb", lab: false }));
  ...
  it("heelStake plants a post on the second step", () => {
    ...
    const b = makeBattle("catcher", makeTestContext({ mode: "climb", lab: false }), run);
  ...
  it("drawcut hits harder when adjacent", () => {
    ...
    const far = makeBattle("catcher", makeTestContext({ mode: "climb", lab: false }), run);
    const uid = far.hand[0].uid;
    const farHit = previewCard(far, uid, makeTestContext({ mode: "climb", lab: false }));
    far.player.pos = 4;
    const nearHit = previewCard(far, uid, makeTestContext({ mode: "climb", lab: false }));
    expect(farHit.enemyHp).toBeGreaterThan(nearHit.enemyHp);
  });

  it("preview still matches settlement for push", () => {
    const b = makeBattle("catcher", makeTestContext({ mode: "climb", lab: false }));
    const push = b.hand.find((c) => c.defId === "push")!;
    const prev = previewCard(b, push.uid, makeTestContext({ mode: "climb", lab: false }));
    const after = playCard(b, push.uid, makeTestContext({ mode: "climb", lab: false }));
    expect(after.enemy.hp).toBe(prev.enemyHp);
    expect(after.enemy.pos).toBe(prev.enemyPos);
  });
  ...
  it("lets a yard bandit take the stone", () => {
    const b = makeBattle("bandit", makeTestContext({ mode: "climb", lab: false }));
  ...
  it("weaves after an attack, mirrors threat, and retains ironform block", () => {
    ...
    let iron = makeBattle("catcher", makeTestContext({ mode: "climb", lab: false }), run2, true);
    iron = playNamed(iron, "ironform");
    expect(iron.playerBlock).toBe(10);
    expect(iron.retainTurns).toBe(2);
    iron = endTurn(iron, makeTestContext({ mode: "climb", lab: false }));
  ...
  it("lets a smuggler raise a guard before winding up", () => {
    const b = makeBattle("smuggler", makeTestContext({ mode: "climb", lab: false }));
  ...
  it("fires a buried slash when the catcher finally hits", () => {
    ...
    expect(statusChips(b, "you", makeTestContext({ mode: "climb", lab: false })).some((c) => c.key === "bury" && c.value.includes("回刀"))).toBe(true);
    b = endTurn(b, makeTestContext({ mode: "climb", lab: false }));
...
describe("先机", () => {
  it("keeps the catcher slower than a palm so the tutorial still opens", () => {
    const b = makeTutorialBattle(makeTestContext({ mode: "climb", lab: false }));
    expect(yourPace(b, makeTestContext({ mode: "climb", lab: false }))).toBe(8); // §31.11 拳最快
  ...
  it("lets a faster foe plant a trap before you move", () => {
    const b = makeBattle("trapper", makeTestContext({ mode: "climb", lab: false }));
    expect(b.foePace).toBeGreaterThan(yourPace(b, makeTestContext({ mode: "climb", lab: false })));
    seizeOpening(b, makeTestContext({ mode: "climb", lab: false }));
  ...
  it("lets 抢先 raise 先机", () => {
    ...
    let b = makeBattle("catcher", makeTestContext({ mode: "climb", lab: false }), run, true);
    expect(yourPace(b, makeTestContext({ mode: "climb", lab: false }))).toBe(8);
    b = playNamed(b, "haste");
    expect(yourPace(b, makeTestContext({ mode: "climb", lab: false }))).toBe(11);
  });
});
...
describe("stance, swap, and mates", () => {
  ...
  it("keeps a buried form for several turns when it does not fire", () => {
    const run = makeRun("empty");
    run.deck = ["burySlash", "defend", "defend", "defend", "defend"];
    let b = makeBattle("smuggler", makeTestContext({ mode: "climb", lab: false }), run, true);
    b = playNamed(b, "burySlash");
  ...
  it("discards off-school pages when a mate takes the floor", () => {
    ...
    b = swapFighter(b, "watch", makeTestContext({ mode: "climb", lab: false }));
