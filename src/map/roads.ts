...
/** 每个驿站独立路网骨架（尺寸/通路形状都不同），禁止「同一十字换贴图」。 */
const ROAD_ASCII: Record<string, string[]> = {
  jiaxing: [
    "##############################################",
    "#~~~~~~~~~~~~~~~~~~#N#~~~~~~~~~~~~~~~~~~~~~..#",
    "#~~~~~~~~~~~~~~~~~~.=.~~~~~~~~~~~~~~~~~~~~~..#",
...
function roadTown(opts: {
  ...
  const base = ROAD_ASCII[opts.id];
  if (!base) throw new Error(`missing unique road ascii for ${opts.id}`);
  const grid = base.map((line) => line.split(""));
...
export const ROAD_SCENES: Record<string, RoadSceneDef> = {
  ...
  chuzhou: roadTown({
    ...
    name: "滁州",
    kicker: "醉翁亭",
    enter: "醉翁亭立在山前。北上淮阴，南下建康。山路有人拦客。",
    mood: "山不高，影却长。亭是城内的眼。",
